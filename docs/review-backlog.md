...
- Flagship case-study framing is no longer an open broad-polish item unless a reviewer says the selected-work section is still hard to evaluate: Turn 60 adds reviewer-path sequences to the existing four flagship cards without changing project order, media policy, evidence claims, routes, or `Legacy` continuity.
- Native hero personality is no longer an open broad-polish item unless a reviewer says the first viewport still feels static: Turn 61 adds a reduced-motion-safe typed working-mode signal using existing phrases, without importing libraries, adding canvas, changing evidence JSON, or touching `/legacy/`.
- Recruiter trust and share surface are no longer open broad-polish items unless a reviewer reports confusion: the final sprint contact route explains how to use selected work, resume, GitHub, and LinkedIn together, and the social preview was regenerated from the current homepage state.
